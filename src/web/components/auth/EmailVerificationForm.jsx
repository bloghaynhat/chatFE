...
export const EmailVerificationForm = ({
  ...
  useEffect(() => {
    ...
    if (fromRegister) {
      setSuccessMessage(
        "Mã OTP đã được gửi từ bước đăng ký. Vui lòng kiểm tra email và nhập mã để xác thực.",
      );
      ...
      const autoSendOtp = async () => {
        try {
          ...
          setSuccessMessage(
            "Tài khoản chưa được xác thực. Chúng tôi đã tự động gửi mã OTP mới đến email của bạn.",
          );
        } catch (err) {
    ...
    if (countdown > 0) {
      ...
      const autoResend = async () => {
        try {
          await resendVerification({ email: email.trim() });
          setSuccessMessage(
            "Mã OTP mới đã tự động được gửi đến email của bạn.",
          );
          setCountdown(60);
        } catch (err) {
        ...
        <div className="flex justify-between items-center mb-1">
          ...
          {countdown > 0 && (
            <span className="text-sm text-gray-500">
              Tự động gửi lại sau {countdown}s
            </span>
          )}
        </div>
        <input
