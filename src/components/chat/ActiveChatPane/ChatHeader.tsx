...
import { AiTasksModal } from "../Ai/AiTasksModal";
...
export const ChatHeader = ({
  ...
  const [isAiTasksModalOpen, setIsAiTasksModalOpen] = useState(false);
  ...
  return (
    <>
      <AiSummaryModal
        ...
        members={selectedChat?.participants || selectedChat?.members || []}
      />
      <AiTasksModal
        isOpen={isAiTasksModalOpen}
        onClose={() => setIsAiTasksModalOpen(false)}
        conversationId={selectedConversationId}
        members={selectedChat?.participants || selectedChat?.members || []}
      />
      <AiSearchModal
            ...
            <div className="flex items-center gap-1 text-gray-500 dark:text-gray-300">
              ...
              <div ref={moreMenuRef} className="relative">
                <button
                  ...
                  className={`h-8 w-8 lg:h-9 lg:w-9 inline-flex items-center justify-center rounded-full transition ${isMoreMenuOpen
                ...
                <div
                  className={`absolute right-0 top-10 w-[260px] max-w-[84vw] rounded-2xl bg-[#edf4f1] dark:bg-slate-800 shadow-2xl p-2 border border-white/70 dark:border-slate-700 z-50 origin-top-right will-change-transform transition-all duration-200 ease-out ${isMoreMenuOpen
                  ...
                  {visibleMoreActions.map((action) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-left text-[14px] leading-none hover:bg-white/75 dark:hover:bg-slate-700/80 transition ${action.danger
                            ? "text-red-500"
                            : "text-gray-900 dark:text-gray-100"
                          }`}
                      >
                        <ActionIcon
                          className={`text-[18px] shrink-0 ${action.id.startsWith("ai-") ? "text-blue-500" : ""
                            }`}
                        />
