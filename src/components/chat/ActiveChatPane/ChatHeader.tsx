...
export const ChatHeader = ({
  ...
  const visibleMoreActions = isSavedMessages
    ? moreActions.filter(
        (action) =>
          !["call", "video-call", "share-contact", "send-gift", "block-user", "disable-sharing"].includes(action.id),
      )
    : moreActions;
    ...
    <div className="px-4 lg:px-5 py-2.5 border-b border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900">
      {!isHeaderSearchOpen ? (
          ...
          >
            ...
            <div className="min-w-0">
              ...
              <p className="text-[11px] text-gray-500 dark:text-gray-400">
                {isSavedMessages ? "Saved messages" : isLoading ? "Opening conversation..." : "last seen 1 hour ago"}
              </p>
            </div>
          </div>
        ...
        <div className="flex items-center gap-2 animate-in fade-in duration-200">
          <div
            className="w-8 h-8 lg:w-9 lg:h-9 rounded-full bg-gradient-to-br from-blue-500 to-blue-600 text-white font-semibold flex items-center justify-center overflow-hidden shrink-0 transition-opacity shadow-sm border border-black/5 dark:border-white/10 cursor-pointer hover:opacity-90"
            onClick={handleToggleInfo}
          >
            {isSavedMessages ? (
