...
import { BlockManagerModal } from "../settings/BlockManagerModal";
import { useState } from "react";

interface SettingsPanelProps {
  isOpen?: boolean;
  onClose?: () => void;
  initialTab?: "main" | "edit-profile";
  isCollapsed?: boolean;
  onBack?: () => void;
...
export const SettingsPanel: React.FC<SettingsPanelProps> = ({
  ...
  const [activeTab, setActiveTab] =
    useState<"main" | "edit-profile">(initialTab);
  const [isBlockManagerOpen, setIsBlockManagerOpen] = useState(false);
  const handleBack = onClose || onBack || (() => undefined);
      ...
      <div className="flex-1 flex flex-col h-full bg-white dark:bg-slate-900 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-4 py-2.5 bg-white dark:bg-slate-900 shadow-sm z-10 shrink-0">
          <div className="flex items-center gap-5">
            <button
              onClick={handleBack}
              className="text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-slate-800 p-2 rounded-full transition -ml-2"
            >
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className="text-gray-500 hover:bg-gray-100 p-2 rounded-full transition dark:text-gray-400 dark:hover:bg-slate-800"
            >
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar bg-white dark:bg-slate-900 pb-8">
          <div className="flex flex-col items-center pt-8 pb-4">
            ...
            <p className="text-[14px] text-blue-500 dark:text-blue-400 font-medium tracking-wide">
              online
            </p>
          </div>
          ...
          <div className="flex flex-col mb-2 border-b border-gray-100/60 dark:border-slate-800 pb-2">
            <div className="flex items-center gap-6 px-5 py-3 hover:bg-gray-50 dark:hover:bg-slate-800 cursor-pointer transition">
              ...
              <div className="flex-1">
                <p className="text-[15px] font-normal text-gray-900 dark:text-white leading-tight mb-0.5">
                  {displayPhone}
                </p>
          ...
          <div className="flex flex-col py-2 border-b border-gray-100/60 dark:border-slate-800">
            ...
            <SettingsMenuItem
              icon={<FiShieldOff />}
              label="Blocked Users"
              onClick={() => setIsBlockManagerOpen(true)}
            />
          ...
          <div className="flex flex-col py-2">
            <div className="flex items-center px-4 py-3.5 hover:bg-gray-50 dark:hover:bg-slate-800 cursor-pointer transition">
              ...
              <span className="text-[15px] text-gray-900 dark:text-white font-medium flex-1">
                Telegram Premium
              </span>
            </div>
          </div>
        ...
        {activeTab === "edit-profile" && (
          <div className="absolute inset-0 z-20 bg-white dark:bg-slate-900 flex flex-col">
            <div className="flex items-center gap-5 px-4 py-2.5 bg-white dark:bg-slate-900 shadow-sm shrink-0">
              <button
            ...
            <div className="flex-1 overflow-y-auto flex items-center justify-center p-6">
              <AvatarEditor
                ...
                onAvatarChange={(avatar: any) =>
                  updateUserProfile({ avatarUrl: avatar.url })
                }
        ...
        <BlockManagerModal
          ...
          onClose={() => setIsBlockManagerOpen(false)}
        />
      </div>
    );
  }
...
const SettingsMenuItem = ({ icon, label, rightText, onClick }: any) => {
  return (
    ...
    >
      ...
      {rightText && (
        <span className="text-[14px] text-gray-400 mr-2">{rightText}</span>
      )}
    </div>
  );
};
